...
# p节点IP
P_IP=('61.47.19.68' '61.47.19.67' '61.47.19.71')
# D节点IP D节点首节点IP
D_IP=('61.47.19.70')
...
do
    ...
    #     export DEEPEP_NORMAL_COMBINE_ENABLE_LONG_SEQ=1
        ...
        --served-model-name glm-5 --chunked-prefill-size 4096 --max-prefill-tokens 180000 --moe-a2a-backend deepep --deepep-mode normal \
...
do
    ...
    then
        ...
        --port 8003 --trust-remote-code --dist-init-addr 61.47.19.70:5000 --nnodes 1 --node-rank $i --tp-size 16 --dp-size 16 --enable-dp-attention --ep-size 16 \
        --mem-fraction-static 0.895 --max-running-requests 128 --attention-backend ascend --device npu --quantization modelslim \
        --served-model-name glm-5 --moe-a2a-backend deepep --deepep-mode low_latency \
        --cuda-graph-max-bs 8 --disaggregation-transfer-backend ascend --watchdog-timeout 9000 --context-length 180000 \
        --tokenizer-worker-num 8 --prefill-round-robin-balance --disable-shared-experts-fusion --dtype bfloat16  --load-balance-method round_robin \
        --speculative-draft-model-quantization unquant \
